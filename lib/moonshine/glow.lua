...
function fonum7(num)
  return string.format("%f", num):gsub(",",".")
end
...
local function make_blur_shader(sigma)
  ...
  local blur_line = "c += vec4(%s) * Texel(texture, tc + vec2(%s) * direction);\n"

  for i = -support,support do
    local coeff = math.exp(-.5 * i*i * one_by_sigma_sq)
    norm = norm + coeff
    code[#code+1] = blur_line:format(fonum7(coeff), fonum7(i))
  end

  code[#code+1] = ("return c * vec4(%s) * color;}"):format(fonum7(1 / norm))
  local conCode = table.concat(code)
  return love.graphics.newShader(conCode)
end
